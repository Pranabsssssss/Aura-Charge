...
if (!isset($_SESSION['user']) || empty($_SESSION['user'])) {
    ?>
    <script>
        // Clear all storage and redirect to signin
        localStorage.clear();
...
function parseSessionData($cellData, $sessionCounter, &$userSessions) {
    ...
    // Pattern 2: Look for any date pattern and extract surrounding data
    if (preg_match('/(\d{1,2}[-\/]\d{1,2}[-\/]\d{2,4})/', $cleanData, $dateMatch)) {
        $parts = preg_split('/\s+/', $cleanData);
        $dateIndex = -1;
        
        // Find the date in the parts array
    ...
    // Pattern 3: Try comma or semicolon separated values
...
// Fetch users.csv data once and use it for both user info and sessions
...
if ($httpCode === 200 && $usersContent !== false) {
    ...
    // Step 1: Find user and get basic info
    $userColumnIndex = -1;
    foreach ($lines as $lineIndex => $line) {
        ...
        // Check if this is the logged-in user (email should be in column 2, index 1)
        if (isset($data[1]) && strcasecmp($data[1], $userEmail) === 0) {
            ...
            $userColumnIndex = $lineIndex; // Store which row this user is in
    ...
    if ($userColumnIndex >= 0) {
        ...
        if (!empty($userRowLine)) {
            ...
            // Extract sessions from column 6 onwards (index 5 onwards) for this specific user
            for ($columnIndex = 5; $columnIndex < count($userRowData); $columnIndex++) {
                if (isset($userRowData[$columnIndex]) && !empty(trim($userRowData[$columnIndex]))) {
                    ...
                    // Parse the sessions data for this specific user
        ...
        // Even if user not found, provide sample data
        $sampleSessions = [
...
// Only use email fallback if CSV file doesn't exist or user not found
if (empty($userName)) {
    $userName = explode('@', $userEmail)[0];
    $userName = ucfirst($userName); // Capitalize first letter
}

// Generate profile picture using UI Avatars
$userPicture = "https://ui-avatars.com/api/?name=" . urlencode($userName) . "&background=00bfff&color=fff&size=128&bold=true";

// Fetch car data from data.csv using the user's serial number
if (!empty($userSerialNo)) {
    ...
    if ($httpCode === 200 && $dataContent !== false) {
        ...
        foreach ($lines as $lineIndex => $line) {
            ...
            // Check if this row's ID matches the user's serial number
    ...
    <header class="main-header">
        <nav class="flex items-center justify-between px-6 py-4">
            ...
            <div id="userProfile" class="profile-container">
                <img id="profilePicture" src="<?php echo htmlspecialchars($userPicture); ?>" alt="Profile" class="profile-picture">
                <div class="flex flex-col">
                    <span id="userName" class="text-sm font-semibold text-white truncate max-w-24 sm:max-w-none"><?php echo htmlspecialchars($userName); ?></span>
                    <span id="userEmail" class="text-xs text-gray-400 truncate max-w-24 sm:max-w-none"><?php echo htmlspecialchars($userEmail); ?></span>
                </div>
                <svg class="w-4 h-4 text-gray-400 ml-2 hidden sm:block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="container mx-auto px-6">
            ...
            <div class="max-w-4xl mx-auto">
                ...
                <div id="carDetails" class="dashboard-card rounded-2xl p-8 fade-in-up">
                    ...
                    <div class="text-center mb-8">
                        <div id="carImageContainer" class="relative inline-block">
                            ...
                            <img id="carImage" src="<?php echo htmlspecialchars($carImage); ?>" alt="Car Image" class="max-w-full sm:max-w-80 max-h-48 sm:max-h-60 object-contain rounded-xl border-2 border-cyan-400/30 shadow-lg" onerror="this.style.display='none'; document.getElementById('carImagePlaceholder').style.display='flex';">
                            <?php endif; ?>
                            <div id="carImagePlaceholder" class="w-full sm:w-80 h-48 sm:h-60 max-w-80 bg-gradient-to-br from-gray-700 to-gray-800 rounded-xl border-2 border-cyan-400/30 flex items-center justify-center mx-auto" <?php echo !empty($carImage) ? 'style="display: none;"' : ''; ?>>
                                <svg class="w-16 sm:w-20 h-16 sm:h-20 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <!-- Car Name -->
                        <div class="text-center p-6 bg-gradient-to-br from-cyan-400/10 to-blue-500/10 rounded-xl border border-cyan-400/20">
                            <div class="w-12 h-12 bg-gradient-to-br from-cyan-400 to-blue-500 rounded-xl flex items-center justify-center mx-auto mb-4">
                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                                </svg>
                            </div>
                            <h3 class="text-lg font-semibold text-gray-400 mb-2">Car Model</h3>
                            <p id="carName" class="text-2xl font-orbitron font-bold gradient-text"><?php echo !empty($carName) ? htmlspecialchars($carName) : 'Not Available'; ?></p>
                        </div>

                        <!-- Car Number -->
                        <div class="text-center p-6 bg-gradient-to-br from-green-400/10 to-emerald-500/10 rounded-xl border border-green-400/20">
                            ...
                            <p id="carNumber" class="text-2xl font-orbitron font-bold gradient-text"><?php echo !empty($carNumber) ? htmlspecialchars($carNumber) : 'Not Available'; ?></p>
                        </div>

                        <!-- Owner Name -->
                        <div class="text-center p-6 bg-gradient-to-br from-purple-400/10 to-pink-500/10 rounded-xl border border-purple-400/20">
                            <div class="w-12 h-12 bg-gradient-to-br from-purple-400 to-pink-500 rounded-xl flex items-center justify-center mx-auto mb-4">
                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                </svg>
                            </div>
                            <h3 class="text-lg font-semibold text-gray-400 mb-2">Owner</h3>
                            <p id="ownerName" class="text-2xl font-orbitron font-bold gradient-text"><?php echo htmlspecialchars($userName); ?></p>
                        </div>
                    </div>
                </div>
                <?php else: ?>
                ...
                <div id="errorMessage" class="error-message rounded-lg p-6 text-center">
                    ...
                    <p class="text-sm">Your account (Serial: <?php echo htmlspecialchars($userSerialNo); ?>) is not linked to any vehicle. Please contact support.</p>
                </div>
                <?php endif; ?>
                ...
                <div class="dashboard-card rounded-2xl p-6 sm:p-8 mt-8 fade-in-up delay-200">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left">
                                ...
                                <thead class="bg-gray-800/50">
                                    <tr>
                                        <th class="px-4 py-3 text-xs font-semibold text-gray-300 uppercase tracking-wider border-b border-gray-700">
                                            #
                                        </th>
                                        <th class="px-4 py-3 text-xs font-semibold text-gray-300 uppercase tracking-wider border-b border-gray-700 min-w-[200px]">
                                            Station Details
                                        </th>
                                        <th class="px-4 py-3 text-xs font-semibold text-gray-300 uppercase tracking-wider border-b border-gray-700 min-w-[120px]">
                                            Date
                                        </th>
                                        <th class="px-4 py-3 text-xs font-semibold text-gray-300 uppercase tracking-wider border-b border-gray-700 min-w-[100px]">
                                            Duration
                                        </th>
                                        <th class="px-4 py-3 text-xs font-semibold text-gray-300 uppercase tracking-wider border-b border-gray-700 min-w-[100px]">
                                            Start Time
                                ...
                                <tbody class="bg-gray-900/20 divide-y divide-gray-700/50">
                                    ...
                                    <tr class="hover:bg-gray-800/30 transition-all duration-200 group">
                                        <!-- Serial Number -->
                                        <td class="px-4 py-4">
                                            <div class="flex items-center justify-center w-8 h-8 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 rounded-full border border-cyan-500/30">
                                                <span class="text-sm font-bold text-cyan-300"><?php echo htmlspecialchars($session['sno']); ?></span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                <div class="min-w-0 flex-1">
                                                    <p class="text-sm font-semibold text-white group-hover:text-cyan-300 transition-colors">
                                        ...
                                        <!-- Duration -->
                                        <td class="px-4 py-4">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-cyan-400/20 to-blue-500/20 border border-cyan-400/30 text-cyan-300">
                    ...
                    <div class="text-center py-16">
                        <div class="mx-auto w-24 h-24 bg-gradient-to-br from-gray-700 to-gray-800 rounded-full flex items-center justify-center mb-6 shadow-lg">
                        ...
                        <div class="max-w-md mx-auto">
                            ...
                            <p class="text-gray-500 mb-6">Your charging history will appear here once you start using our charging stations.</p>
                            <div class="bg-gradient-to-r from-cyan-400/10 to-blue-500/10 border border-cyan-400/20 rounded-lg p-4">
...
requestAnimationFrame(raf);
        ...
        // Logout function - clears all data and redirects
        function logout() {
...
<script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'99213f43172d8af1',t:'MTc2MTA1NDg2OS4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();</script></body>
</html>
